...
(function (d3, $, queue, window) {
	...
	// .defer(d3.json, "./UgandaDistricts.geojson")//DNAME_06
		.defer(d3.json, "./data/UgandaDistricts.geojson")
		.defer(d3.csv, "./data/mapValues.csv")
		.defer(d3.csv, "./data/Cash_4Ws_April_2018.csv")
		.await(ready);
	...
	function ready(error, ugandaGeoJson, sector, relationship) {
		...
		var beneficiaries = d3.sum(relationship, function(d){return parseFloat(d["HH 2"])});
		...
		logo.onAdd = function(map){
			...
			div.innerHTML= "<a href='https://geogecko.com/' target='_blank'><img style='right: 0; width: 12em' src='css/logo-full-no-text.svg' alt='GeoGecko'></a>";
			return div;
		}
		logo.addTo(map);
	...
	maxZoom: 19
		...
		function updateTable(data) {
			...
			var titlesText = ["District Name", "Number of agencies"]
			...
			rows.selectAll('td')
				...
				if (d.name === "key") {
					var parishDataFilter = districtList.filter(function (k) {
						if (d.value === k.key) {
							var str = "<thead><tr><th style='border: 1px solid #ccc!important;text-decoration: none !important; text-align: left;'>Agency Name</th> <th style='text-decoration: none !important; border: 1px solid #ccc!important; text-align: left;'>Project Title</th><th style='text-decoration: none !important; border: 1px solid #ccc!important; text-align: left;'>Delivery Mechanism</th><th style='text-decoration: none !important; border: 1px solid #ccc!important; text-align: left;'>Project Start</th><th style='text-decoration: none !important; border: 1px solid #ccc!important; text-align: left;'>Project End</th></tr></thead>";

							var tooltipList = "";
							var i = 0;
							while (i < k.values.length) {
								tooltipList = tooltipList + ("<tr><td style='border: 1px solid #ccc!important;text-decoration: none !important; text-align: left;'>" + k.values[i].Organisation + "</td> <td style='border: 1px solid #ccc!important; text-decoration: none !important; text-align: left;'>" + k.values[i]["Type of activity"] + "</td><td style='border: 1px solid #ccc!important; text-decoration: none !important; width: 2em!important; text-align: left;'>" + k.values[i]["Delivery Mechanism"] + "</td><td style='border: 1px solid #ccc!important; text-decoration: none !important; text-align: left;'>" + k.values[i]["Start date"] + "</td><td style='border: 1px solid #ccc!important; text-decoration: none !important; width: 2em!important; text-align: left;'>" + k.values[i]["End date"] + "</td></tr>");
								i++
							}					
			...
			rows.selectAll('td')
				...
				if (d.name === "key") {
					var agencyDataFilter = agencyList.filter(function (k) {
						if (d.value === k.key) {
							var str = "<thead><tr><th style='border: 1px solid #ccc!important; text-decoration: none !important; text-align: left;'>District</th> <th style='border: 1px solid #ccc!important; text-decoration: none !important; text-align: left;'>Project Title</th><th style='text-decoration: none !important; border: 1px solid #ccc!important; text-align: left;'>Delivery Mechanism</th><th style='border: 1px solid #ccc!important; text-decoration: none !important; border: 1px solid #ccc!important; text-align: left;'>Start date</th><th style='border: 1px solid #ccc!important; text-decoration: none !important; border: 1px solid #ccc!important; text-align: left;'>End date</th></tr></thead>";

							var tooltipList = "";
							var i = 0;
							while (i < k.values.length) {
								tooltipList = tooltipList + ("<tr><td style='border: 1px solid #ccc!important; text-decoration: none !important; text-align: left;'>" + k.values[i]["District"] + "</td> <td style='border: 1px solid #ccc!important; text-decoration: none !important; text-align: left;'>" + k.values[i]["Type of activity"] + "</td><td style='border: 1px solid #ccc!important; text-decoration: none !important; text-align: left;'>" + k.values[i]["Delivery Mechanism"] + "</td><td style='border: 1px solid #ccc!important; text-decoration: none !important; text-align: left;'>" + k.values[i]["Start date"] + "</td><td style='border: 1px solid #ccc!important; text-decoration: none !important; width: 2em!important; text-align: left;'>" + k.values[i]["End date"] + "</td></tr>");
								i++
							}					
							document.getElementById('modal-header').innerHTML = d.value;
							document.getElementById('modal-body').innerHTML = str + tooltipList;
		...
		var countriesOverlay = L.d3SvgOverlay(function (sel, proj) {
			...
			ugandaPath.enter()
				...
				var str = "<tr><button type='button' class='close' onclick='$(this).parent().hide();'>×</button></tr>" +
					"<th><br/></th><tr><th>District:</th> <th style='right: 0;'><b>" + d.properties.DNAME_06 + "</b></th></tr>";
		...
		function myFilter(c, flag, needRemove) {
			...
			var selectedDataset = dataset.filter(function (d) { //global.selectedDataset
				...
				if (global.selectedAgency.length > 0) {
					global.selectedAgency.map(function (c) {
						if (c.values[0].Organisation === d.Organisation) {
							isAgency = true;
						}
					});
				} else {
			...
			ugandaPath.each(function (d) {
				...
				selectedDataset.map(function(c) {
					if (c.District === d.properties.DNAME_06) {
			...
			beneficiaries = d3.sum(selectedDataset, function(d){return parseFloat(d["HH 2"])});
